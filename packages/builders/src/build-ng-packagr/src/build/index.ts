...
import {AssetPattern} from './assets-patterns';
...
function checkNgPackagrVersion(projectRoot: string): boolean {
  ...
  try {
    ngPackagrJsonPath = resolveProjectModule(
      projectRoot,
...
export class NgPackagrBuilder implements Builder<NgPackagrBuilderOptions> {
  ...
  run(
    ...
    if (!options.project) {
      throw new Error(
        'A "project" must be specified to build a library\'s npm package.',
      );
    }
    ...
    return new Observable(obs => {
      ...
      const packageJsonPath = getSystemPath(
        resolve(root, normalize(options.project)),
      );

      const ngPkgProject = projectNgPackagr
        .ngPackagr()
        .forProject(packageJsonPath);

      if (options.tsConfig) {
        const tsConfigPath = getSystemPath(
          resolve(root, normalize(options.tsConfig)),
        );
      ...
      if (options.watch) {
        ...
        const ngPkgSubscription = ngPkgProject
          ...
          .pipe(
            tap(() => {
              handleAssets(this.context, packageJsonPath, builderConfig).then(
